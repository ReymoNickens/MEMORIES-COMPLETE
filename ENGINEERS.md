...
1. Run Supabase migrations `001` through `017` in order. `017` is the audit
   hardening pass and is **not optional** — it enables RLS on the payroll and
...
4. Point Paystack webhook at `/api/webhooks/paystack`. There is no Edge
...
6. Provision each door scanner with **two** localStorage values: `device_id`
...
`.github/workflows/ci.yml` type-checks every workspace, runs the shared unit
tests, applies all migrations against a real Postgres, runs the demo night, and
then asserts the things that matter: that `anon` cannot read payroll or call
